...
def deep_CNN(images, batch_size, n_classes):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        b_conv1 = tf.Variable(bias_variable([32]), name='biases', dtype=tf.float32)  # 64个偏置值
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        b_conv2 = tf.Variable(bias_variable([64]), name='biases', dtype=tf.float32)  # 32个偏置值
    ...
    with tf.variable_scope('conv3') as scope:
        w_conv3 = tf.Variable(weight_variable([3, 3, 64, 128], 0.1), name='weights', dtype=tf.float32)

        b_conv3 = tf.Variable(bias_variable([128]), name='biases', dtype=tf.float32)  # 16个偏置值
    ...
    # 将前面的FC层输出，做一个线性回归，计算出每一类的得分，在这里是5类，所以这个层输出的是5个得分。
